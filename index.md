...
- [ChangingStatus](pages/user/ChangingStatus.md) — переход заявки по стадиям, возврат закрытой заявки в работу и мягкое удаление (скрытие из списка по правам).
...
- [Filters](pages/user/Filters.md) — поиск Заявок: активные фильтры, отбор по полям, настройка панели фильтров, быстрые фильтры (в т.ч. в мобильном), фильтрация закрытых и удалённых Заявок, строка поиска.
- [GroupActionsForTickets](pages/user/GroupActionsForTickets.md) — массовые действия над списком заявок: назначение исполнителей, смена стадии (до 500 за раз), возврат закрытых заявок в работу и удаление с возможностью найти удалённые через фильтр.
- [HistoryOfChanges](pages/user/HistoryOfChanges.md) — вкладка «История изменений» заявки: хронология событий по всем вкладкам (стадии, поля, материалы, чек-листы, акт), столбец «Местоположение» с картой и боковое окно с деталями изменения.
...
- [DeletedObjects](pages/user/DeletedObjects.md) — удаление/блокировка компаний, объектов, заявок, чек-листов, сотрудников и заказчиков; заявки можно восстановить, остальное только пересоздать, плюс почему прямого удаления навсегда нет и как скрыть удалённое.
- [FloorPlan](pages/user/FloorPlan.md) — план помещения объекта и указание расположения объекта на плане при подаче заявки.
- [HowToMakePassport](pages/user/HowToMakePassport.md) — создание паспорта объекта в web через привязку QR-кода к объекту и шаблону заявки, подача заявки сканированием с телефона и заказ индивидуального электронного паспорта по запросу.
...
- [CreatingCompany](pages/user/CreatingCompany.md) — создание карточки компании: виды, тип контрагента, контактные лица, юр. и банковские реквизиты, договоры со сроками и объектами, импорт/экспорт, ERP ID для интеграций.
- [CreatingCustomer](pages/user/CreatingCustomer.md) — создание карточки заказчика и перевод заказчика в сотрудники.
- [CreatingUser](pages/user/CreatingUser.md) — создание карточки сотрудника: поля, мобильный сотрудник, отслеживание местоположения и точность геокоординат, SSO, роль, участок, виды работ, график; бан, удаление и перевод сотрудника в заказчики.
...
- [Roles](pages/admin/Roles.md) — роль как набор прав; базовые роли (Заказчик, Сервисный специалист, Диспетчер), создание и настройка роли (полномочия, приложения, плагины), уровни доступа и настройка видимости данных по участкам.
...
- [Messages](pages/user/Messages.md) — обмен сообщениями по заявке в web и мобильном приложениях: добавление и удаление участников чата, отправка фото/видео, разделение на чат с командой и чат с заказчиком (по запросу, с настройкой доступа по ролям).
...
- [AdditionalFields](pages/user/AdditionalFields.md) — создание в консоли администратора и использование дополнительных полей (разные типы, включая значение из справочника) для компаний, объектов, заявок, работ, чек-листов, договоров, сотрудников и заказчиков.
...
- [MobileDevice](pages/user/MobileDevice.md) — технические требования и рекомендованные модели Android/iOS-устройств для мобильных приложений HubEx, предупреждение об отказе от Huawei и поддержка системного масштаба шрифта в приложении «Исполнитель».
...
- [KnowledgeBase](pages/user/KnowledgeBase.md) — плагин базы знаний HubEx: подключение через Магазин плагинов, дерево разделов и статей, поиск по заголовкам и содержимому, блочный редактор статей, настройка доступа к статьям по ролям HubEx.
...
- [Список заявок: карточное и табличное представление](pages/user/TicketsListView.md) — как переключать список заявок между карточным и табличным видом, настраивать отображаемые поля и их порядок и открывать заявку в боковом окне справа без перезагрузки страницы.
- [Конструктор формы заявки](pages/admin/TicketFormBuilder.md) — как настроить расположение полей на форме заявки для каждого типа заявки: группировать поля в блоки, переименовывать, перемещать и удалять блоки, менять порядок полей и восстанавливать форму по умолчанию.
- [Магазин плагинов](pages/admin/PluginMarketplace.md) — встроенный каталог дополнений в консоли администратора: поиск и категории, подключение/отключение плагинов в один клик, открытие доступа по ролям и голосование за плагины со статусом «Скоро»; доступен на тарифах выше «Минимальный».
